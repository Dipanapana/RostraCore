...
NOTE: The setup_wizard_data column is already included in migration 019 (Plan 00-01 Task 3).
...
4. Test industry-specific role previews are visible on each card:
   - **Hospitality card**: Should show "waiter" or "chef" in role preview
   - **Retail card**: Should show "cashier" in role preview
   - **Non-Profit card**: Should show "volunteer" in role preview
   - **Government card**: Should show "department_head" or "director" in role preview
5. Select "Hospitality" and click Continue
6. **Step 2**: Enter company name "Test Restaurant", code "TEST-REST", click Continue
7. **Step 3**: Leave "Skip for now" checked, click Continue
8. **Step 4**: Enter admin details (name, email, password), click Continue
9. **Step 5**: Review summary, click "Create Organization"
10. Verify redirect to /dashboard with success message
11. **Refresh test**: Start wizard again, progress partway, refresh browser - verify state is preserved
12. **Time test**: Complete entire wizard - should take under 5 minutes

**CRITICAL - Verify Industry-Specific Roles Load Correctly:**
After completing wizard with Hospitality, check database:
```sql
SELECT industry_template_id FROM organizations WHERE org_code = 'TEST-REST';
-- Should return 'hospitality'
```

Then verify role loading via API:
```bash
curl http://localhost:8000/api/setup-wizard/templates | jq '.[] | select(.template_id=="hospitality") | .preview'
# Should include "waiter" role

curl http://localhost:8000/api/setup-wizard/templates | jq '.[] | select(.template_id=="retail") | .preview'
# Should include "cashier" role

curl http://localhost:8000/api/setup-wizard/templates | jq '.[] | select(.template_id=="nonprofit") | .preview'
# Should include "volunteer" role

curl http://localhost:8000/api/setup-wizard/templates | jq '.[] | select(.template_id=="government") | .preview'
# Should include "department_head" or hierarchy-related role
```

Report any issues with:
- Missing industry cards
- Industry cards not showing correct role previews
- Broken navigation (back/forward)
- Draft state not persisting
- Completion errors
- UI/UX problems
  </how-to-verify>
  <resume-signal>Type "approved" if wizard works correctly AND industry-specific roles are visible, or describe issues found</resume-signal>
</task>
...
- [ ] Hospitality template loads waiter/chef roles in preview
- [ ] Retail template loads cashier role in preview
- [ ] Non-Profit template loads volunteer role in preview
- [ ] Government template loads department hierarchy roles in preview
