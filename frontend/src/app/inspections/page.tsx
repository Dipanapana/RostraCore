                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {inspections.map(insp => {
                      ...
                      const Icon = cfg.icon
                      return (
                        <tr key={insp.inspection_id} className="hover:bg-gray-50">
                          <td className="px-4 py-3">
                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${cfg.color}`}>
                              <Icon className="h-3 w-3" /> {cfg.label}
                            </span>
                          </td>
                          <td className="px-4 py-3 font-medium text-gray-900">{insp.template_name}</td>
                          <td className="px-4 py-3 text-gray-600">{insp.site_name}</td>
                          <td className="px-4 py-3 text-gray-600">{insp.employee_name}</td>
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {tmplLoading ? (
              ...
              <div key={tmpl.template_id} className={`bg-white rounded-xl border p-4 ${tmpl.is_active ? 'border-gray-200' : 'border-gray-100 opacity-60'}`}>
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="font-semibold text-gray-900">{tmpl.name}</h3>
                    <p className="text-xs text-gray-500 mt-0.5">{tmpl.site_name}</p>
                  </div>
                ...
                {tmpl.description && <p className="text-sm text-gray-500 mt-2">{tmpl.description}</p>}
                <div className="mt-3 space-y-1">
                  {(tmpl.items || []).map((item, i) => (
                    <div key={i} className="flex items-center gap-2 text-xs">
                      <div className={`w-3.5 h-3.5 rounded border ${item.required ? 'border-teal-500' : 'border-gray-300'}`} />
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
              <div className="bg-white rounded-xl shadow-xl w-full max-w-xl p-6 max-h-[85vh] overflow-y-auto">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-lg font-semibold text-gray-900">New Inspection Template</h2>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                      ...
                      {tmplForm.items.map((item, idx) => (
                        <div key={idx} className="flex items-center gap-2">
                          <input
                            ...
                            className="flex-1 border border-gray-300 rounded-lg px-3 py-1.5 bg-white text-gray-900 text-sm"
                    ...
                    <button onClick={addItem} className="mt-2 text-sm text-teal-600 hover:underline flex items-center gap-1">
            ...
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                <div className="bg-white rounded-xl border border-gray-200 p-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900 mt-1">{dashboard.total}</p>
                </div>
                <div className="bg-white rounded-xl border border-gray-200 p-4">
                  <p className="text-xs text-emerald-600 uppercase tracking-wide">Completed</p>
                  <p className="text-2xl font-bold text-emerald-600 mt-1">{dashboard.completed}</p>
                </div>
                <div className="bg-white rounded-xl border border-gray-200 p-4">
                  <p className="text-xs text-red-600 uppercase tracking-wide">Flagged</p>
                  <p className="text-2xl font-bold text-red-600 mt-1">{dashboard.flagged}</p>
                </div>
                <div className="bg-white rounded-xl border border-gray-200 p-4">
                  <p className="text-xs text-amber-600 uppercase tracking-wide">In Progress</p>
                  <p className="text-2xl font-bold text-amber-600 mt-1">{dashboard.in_progress}</p>
                </div>
                <div className="bg-white rounded-xl border border-gray-200 p-4">
                  <p className="text-xs text-blue-600 uppercase tracking-wide">Avg Score</p>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {dashboard.sites.map(s => (
                        <tr key={s.site_id} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-2 text-gray-600">{s.total}</td>
                          <td className="px-4 py-2 text-emerald-600">{s.completed}</td>
                          <td className="px-4 py-2 text-red-600">{s.flagged}</td>
