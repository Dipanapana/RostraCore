...
export default function SubscriptionPlansPage() {
  ...
  const fetchPlans = async () => {
    try {
      const response = await fetch(`${getApiUrl()}/api/v1/subscription-plans/plans`, {
        headers: {
  ...
  const handleUpdatePlan = async () => {
    ...
    try {
      const response = await fetch(
        `${getApiUrl()}/api/v1/subscription-plans/plans/${editingPlan.plan_id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleToggleActive = async (planId: number) => {
    try {
      const response = await fetch(
        `${getApiUrl()}/api/v1/subscription-plans/plans/${planId}/toggle-active`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("superadmin_token")}`,
