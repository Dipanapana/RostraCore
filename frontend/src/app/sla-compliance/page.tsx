...
function statusBadge(s: string) {
  if (s === 'compliant') return <span className="px-2 py-0.5 text-xs rounded-full bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400">Compliant</span>
  if (s === 'at_risk') return <span className="px-2 py-0.5 text-xs rounded-full bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400">At Risk</span>
  if (s === 'breached') return <span className="px-2 py-0.5 text-xs rounded-full bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400">Breached</span>
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {tab === 'records' && (
        <>
          ...
          <div className="flex flex-wrap items-center gap-3 mb-4">
            <select value={filterSite} onChange={e => setFilterSite(e.target.value)} className="border rounded-lg px-3 py-2 text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white">
            ...
            <input type="number" value={filterYear} onChange={e => setFilterYear(Number(e.target.value))} className="border rounded-lg px-3 py-2 text-sm w-24 dark:bg-gray-800 dark:border-gray-600 dark:text-white" />
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 dark:bg-gray-700">
                <tr>
                  ...
                  <th className="px-4 py-3 text-center font-medium text-gray-600 dark:text-gray-300">Coverage</th>
                  <th className="px-4 py-3 text-center font-medium text-gray-600 dark:text-gray-300">Response</th>
                  <th className="px-4 py-3 text-center font-medium text-gray-600 dark:text-gray-300">Resolution</th>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                ...
                {records.map((r: any) => (
                  <tr key={r.record_id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    <td className="px-4 py-3 font-medium text-gray-900 dark:text-white">{r.site_name || 'N/A'}</td>
                    <td className="px-4 py-3 text-gray-600 dark:text-gray-400">{MONTHS[r.period_month - 1]} {r.period_year}</td>
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className="text-lg font-bold text-gray-900 dark:text-white">{r.overall_score ?? '—'}</span>
                    </td>
      ...
      {tab === 'dashboard' && dashboard && (
        <div className="space-y-6">
          ...
          {dashboard.avg_coverage != null && (
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-lg font-bold text-gray-900 dark:text-white">{dashboard.avg_coverage}%</span>
              </div>
            </div>
          )}
          ...
          {dashboard.by_site?.length > 0 && (
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
              <h3 className="font-semibold text-gray-900 dark:text-white mb-3">SLA by Site (Latest Month)</h3>
                ...
                {dashboard.by_site.map((s: any) => (
                  <div key={s.site_id} className="flex items-center justify-between py-2 border-b border-gray-100 dark:border-gray-700 last:border-0">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-bold text-gray-900 dark:text-white">{s.overall_score ?? '—'}</span>
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <h2 className="text-lg font-bold mb-4 text-gray-900 dark:text-white">New SLA Record</h2>
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={form.period_month} onChange={e => setForm({...form, period_month: Number(e.target.value)})} className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                    {MONTHS.map((m, i) => <option key={i} value={i + 1}>{m}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Year</label>
                  <input type="number" value={form.period_year} onChange={e => setForm({...form, period_year: Number(e.target.value)})} className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                </div>
              </div>

              <h3 className="text-sm font-semibold text-gray-600 dark:text-gray-400 pt-2">Coverage (%)</h3>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Target</label>
                  <input type="number" step="0.1" value={form.coverage_target} onChange={e => setForm({...form, coverage_target: Number(e.target.value)})} className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                </div>
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Actual</label>
                  <input type="number" step="0.1" value={form.coverage_actual} onChange={e => setForm({...form, coverage_actual: e.target.value})} placeholder="—" className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                </div>
              </div>

              <h3 className="text-sm font-semibold text-gray-600 dark:text-gray-400 pt-2">Response Time (minutes)</h3>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Target</label>
                  <input type="number" value={form.response_target_mins} onChange={e => setForm({...form, response_target_mins: Number(e.target.value)})} className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                </div>
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Avg Actual</label>
                  <input type="number" value={form.response_avg_mins} onChange={e => setForm({...form, response_avg_mins: e.target.value})} placeholder="—" className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                </div>
              </div>

              <h3 className="text-sm font-semibold text-gray-600 dark:text-gray-400 pt-2">Incident Resolution (hours)</h3>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Target</label>
                  <input type="number" value={form.resolution_target_hrs} onChange={e => setForm({...form, resolution_target_hrs: Number(e.target.value)})} className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                </div>
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Avg Actual</label>
                  <input type="number" value={form.resolution_avg_hrs} onChange={e => setForm({...form, resolution_avg_hrs: e.target.value})} placeholder="—" className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                </div>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Notes</label>
                <textarea value={form.notes} onChange={e => setForm({...form, notes: e.target.value})} rows={2} className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
              </div>
            </div>
      ...
      {showEdit && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <h2 className="text-lg font-bold mb-4 text-gray-900 dark:text-white">Edit SLA — {showEdit.site_name} ({MONTHS[showEdit.period_month - 1]} {showEdit.period_year})</h2>
            <div className="space-y-3">
              <h3 className="text-sm font-semibold text-gray-600 dark:text-gray-400">Coverage (%)</h3>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Target</label>
                  <input type="number" step="0.1" value={form.coverage_target} onChange={e => setForm({...form, coverage_target: Number(e.target.value)})} className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                ...
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Actual</label>
                  <input type="number" step="0.1" value={form.coverage_actual} onChange={e => setForm({...form, coverage_actual: e.target.value})} className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Avg Actual</label>
                  <input type="number" value={form.response_avg_mins} onChange={e => setForm({...form, response_avg_mins: e.target.value})} className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Avg Actual</label>
                  <input type="number" value={form.resolution_avg_hrs} onChange={e => setForm({...form, resolution_avg_hrs: e.target.value})} className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                </div>
              </div>
            ...
            <div className="flex justify-end gap-2 mt-4">
              <button onClick={() => setShowEdit(null)} className="px-4 py-2 text-sm text-gray-600 hover:text-gray-800 dark:text-gray-400">Cancel</button>
