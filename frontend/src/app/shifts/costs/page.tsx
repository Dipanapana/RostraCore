    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Shift Cost Breakdown</h1>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
          ...
          <div className="text-xl font-bold text-gray-900 dark:text-white">{formatCurrency(data.total_cost)}</div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
          <div className="text-sm text-gray-500">Assignments</div>
          <div className="text-2xl font-bold text-gray-900 dark:text-white">{data.total_assignments}</div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
          <div className="flex items-center gap-2 text-sm text-gray-500"><Clock className="w-4 h-4" />Total Hours</div>
          <div className="text-xl font-bold text-gray-900 dark:text-white">{data.total_hours}h</div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
          ...
          <div className="flex flex-wrap gap-4">
            {costBreakdown.map(c => (
              <div key={c.label} className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-600 dark:text-gray-400">{c.label}: {formatCurrency(c.value)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {data.by_site.length > 0 && (
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
            <h2 className="font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2"><MapPin className="w-4 h-4" />Cost by Site</h2>
        ...
        {data.by_month.length > 0 && (
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
            <h2 className="font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2"><TrendingUp className="w-4 h-4" />Monthly Trend</h2>
                ...
                <tbody>
                  {data.by_month.map((m: any) => (
                    <tr key={m.month} className="border-b border-gray-100 dark:border-gray-700 last:border-0">
                      ...
                      <td className="py-2 text-right font-bold text-gray-900 dark:text-white">{formatCurrency(m.total_cost)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {data.top_employees.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
          <h2 className="font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2"><Users className="w-4 h-4" />Highest-Cost Employees</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-gray-500 border-b border-gray-200 dark:border-gray-700">
              ...
              <tbody>
                {data.top_employees.map((e: any, i: number) => (
                  <tr key={e.employee_id} className="border-b border-gray-100 dark:border-gray-700 last:border-0">
                    <td className="py-2">
                      <span className="w-6 h-6 inline-flex items-center justify-center rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 text-xs font-bold">{i + 1}</span>
                    </td>
                    <td className="py-2 font-medium text-gray-900 dark:text-white">{e.employee_name}</td>
                    ...
                    <td className="py-2 text-right font-bold text-gray-900 dark:text-white">{formatCurrency(e.total_cost)}</td>
