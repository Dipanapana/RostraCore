  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Key Holding</h1>
      ...
      {summary && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-700">
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Total Keys</p>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-700">
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Issued</p>
            <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">{summary.issued}</p>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-700">
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Lost</p>
            <p className="text-2xl font-bold text-red-600 dark:text-red-400">{summary.lost}</p>
          </div>
        </div>
      )}
      ...
      <div className="flex gap-3 items-center flex-wrap">
        ...
        <span className="text-sm text-gray-500 dark:text-gray-400">{keyTotal} key{keyTotal !== 1 ? 's' : ''}</span>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200 dark:border-gray-700 text-left">
              ...
              <th className="px-4 py-3 font-medium text-gray-500 dark:text-gray-400">Holder</th>
          ...
          <tbody>
            {keys.length === 0 ? (
              ...
              keys.map((k) => (
                <tr key={k.key_id} className="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-750">
                  ...
                  <td className="px-4 py-3 text-gray-600 dark:text-gray-300 capitalize">{k.key_type.replace('_', ' ')}</td>
                  <td className="px-4 py-3 text-gray-600 dark:text-gray-300">{k.site_name}</td>
                  <td className="px-4 py-3 text-gray-500 dark:text-gray-400 max-w-[200px] truncate">{k.description || '-'}</td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusColor(k.status)}`}>
                      {k.status}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-gray-600 dark:text-gray-300">{k.issued_to_name || '-'}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      {k.status === 'available' && (
                        <button
      ...
      {summary && summary.sites.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4">
          <h3 className="font-semibold text-gray-900 dark:text-white mb-3">Keys by Site</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {summary.sites.map((s) => (
              <div key={s.site_id} className="flex items-center justify-between py-2 px-3 bg-gray-50 dark:bg-gray-750 rounded-lg">
                <span className="text-sm text-gray-700 dark:text-gray-300 font-medium">{s.site_name}</span>
                <div className="flex items-center gap-2 text-xs">
                  <span className="text-green-600 dark:text-green-400">{s.available} avail</span>
                  <span className="text-blue-600 dark:text-blue-400">{s.issued} out</span>
                  {s.lost > 0 && <span className="text-red-600 dark:text-red-400">{s.lost} lost</span>}
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md">
            <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
              <h3 className="font-semibold text-gray-900 dark:text-white">Register Key</h3>
      ...
      {issueModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md">
            <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
              <h3 className="font-semibold text-gray-900 dark:text-white">Issue Key</h3>
            ...
            <div className="p-4 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Issue to Employee *</label>
                <select
            ...
            <div className="flex justify-end gap-2 p-4 border-t border-gray-200 dark:border-gray-700">
              <button onClick={() => setIssueModal(null)} className="px-4 py-2 text-sm text-gray-600 dark:text-gray-400">Cancel</button>
              <button
                onClick={handleIssue}
      ...
      {historyModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md">
            <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
              <h3 className="font-semibold text-gray-900 dark:text-white">Key History — {historyModal.key_number}</h3>
            ...
            <div className="p-4 max-h-80 overflow-y-auto">
              {historyLog.length === 0 ? (
                ...
                <div className="space-y-3">
                  {historyLog.map((log) => (
                    <div key={log.log_id} className="flex items-start gap-3 py-2 border-b border-gray-100 dark:border-gray-700 last:border-0">
                      <div className={`p-1.5 rounded-full mt-0.5 ${
                        log.action === 'issued' ? 'bg-blue-100 dark:bg-blue-900/30' :
                        log.action === 'returned' ? 'bg-green-100 dark:bg-green-900/30' :
                        'bg-red-100 dark:bg-red-900/30'
                      }`}>
                        {log.action === 'issued' ? <ArrowRightLeft className="w-3 h-3 text-blue-600 dark:text-blue-400" /> :
                         log.action === 'returned' ? <CheckCircle className="w-3 h-3 text-green-600 dark:text-green-400" /> :
                         <AlertTriangle className="w-3 h-3 text-red-600 dark:text-red-400" />}
                      </div>
                      <div className="flex-1">
                        <p className="text-sm text-gray-900 dark:text-white">
                          <span className="font-medium capitalize">{log.action}</span>
                          {log.employee_name && <span className="text-gray-500 dark:text-gray-400"> — {log.employee_name}</span>}
                        </p>
            ...
            <div className="flex justify-end p-4 border-t border-gray-200 dark:border-gray-700">
              <button onClick={() => setHistoryModal(null)} className="px-4 py-2 text-sm text-gray-600 dark:text-gray-400">Close</button>
            </div>
          </div>
        </div>
      )}
