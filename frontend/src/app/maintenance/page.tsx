    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Site maintenance and repair tracking</p>
        </div>
          ...
          {[
            ...
            <button key={key} onClick={() => setTab(key as any)}
              className={`flex items-center gap-2 py-3 px-1 border-b-2 text-sm font-medium transition-colors ${
                tab === key ? 'border-blue-600 text-blue-600'
                ...
                {items.length === 0 ? (
                  ...
                  items.map((r) => (
                    <tr key={r.request_id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-sm text-gray-600">{r.site_name || '-'}</td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-sm text-gray-600">{r.assigned_to || '-'}</td>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
              <p className="text-xs text-gray-500 mb-1">Estimated Costs</p>
              <p className="text-2xl font-bold text-gray-900">{formatZAR(dashboard.total_estimated_cost)}</p>
            </div>
            <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
              <p className="text-xs text-gray-500 mb-1">Actual Costs (Completed)</p>
              <p className="text-2xl font-bold text-gray-900">{formatZAR(dashboard.total_actual_cost)}</p>
            </div>
          </div>
                  ...
                  {dashboard.by_site.map((s) => (
                    <div key={s.site_id} className="flex items-center justify-between py-1.5">
                      ...
                      <div className="flex items-center gap-2">
                        {s.open > 0 && <span className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">{s.open} open</span>}
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
            ...
            <div className="p-4 space-y-4">
              <div className="bg-gray-50 rounded-lg p-3">
                <p className="text-sm font-medium text-gray-900">{editItem.title}</p>
                <p className="text-xs text-gray-500">{editItem.site_name || 'No site'} | {editItem.category || 'N/A'}</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
