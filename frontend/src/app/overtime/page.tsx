...
function statusBadge(s: string) {
  if (s === 'approved') return <span className="px-2 py-0.5 text-xs rounded-full bg-emerald-50 text-emerald-700">Approved</span>
  if (s === 'rejected') return <span className="px-2 py-0.5 text-xs rounded-full bg-red-100 text-red-700">Rejected</span>
  return <span className="px-2 py-0.5 text-xs rounded-full bg-amber-50 text-amber-700">Pending</span>
}
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-semibold text-gray-900">Overtime Management</h1>
        </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-center font-medium text-gray-600">Hours</th>
              ...
              <tbody className="divide-y divide-gray-100">
                ...
                {records.map((r: any) => (
                  <tr key={r.record_id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-center font-medium text-gray-900">{r.hours}h</td>
                    <td className="px-4 py-3 text-center text-gray-500">{r.rate_multiplier}x</td>
                    <td className="px-4 py-3 text-right font-medium text-gray-900">{formatZAR(r.total_cost)}</td>
      ...
      {tab === 'dashboard' && dashboard && (
        <div className="space-y-6">
          ...
          {dashboard.weekly_trend?.length > 0 && (
            <div className="bg-white rounded-xl shadow p-4">
              <h3 className="font-semibold text-gray-900 mb-3">Weekly Trend (Last 4 Weeks)</h3>
          ...
          {dashboard.top_employees?.length > 0 && (
            <div className="bg-white rounded-xl shadow p-4">
              <h3 className="font-semibold text-gray-900 mb-3">Top Overtime Employees</h3>
                ...
                {dashboard.top_employees.map((e: any, i: number) => (
                  <div key={e.employee_id} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                    <div className="flex items-center gap-2">
                      <span className="w-5 h-5 flex items-center justify-center text-xs bg-orange-100 rounded-full text-orange-700">{i + 1}</span>
          ...
          {dashboard.by_site?.length > 0 && (
            <div className="bg-white rounded-xl shadow p-4">
              <h3 className="font-semibold text-gray-900 mb-3">Overtime by Site</h3>
              <div className="space-y-2">
                {dashboard.by_site.map((s: any) => (
                  <div key={s.site_id || 'none'} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                    <span className="text-sm font-medium text-gray-900">{s.site_name}</span>
                    <span className="text-sm font-bold text-gray-600">{s.hours}h</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg">
            <h2 className="text-lg font-semibold mb-4 text-gray-900">Log Overtime</h2>
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  <input type="number" step="0.5" value={form.hours} onChange={e => setForm({...form, hours: e.target.value})} placeholder="e.g. 3" className="w-full border rounded-lg px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Rate Multi</label>
                  <select value={form.rate_multiplier} onChange={e => setForm({...form, rate_multiplier: e.target.value})} className="w-full border rounded-lg px-3 py-2 text-sm">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Hourly Rate (ZAR, optional)</label>
                <input type="number" step="0.01" value={form.hourly_rate} onChange={e => setForm({...form, hourly_rate: e.target.value})} placeholder="Uses employee rate if blank" className="w-full border rounded-lg px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Reason</label>
                <input type="text" value={form.reason} onChange={e => setForm({...form, reason: e.target.value})} placeholder="Why was overtime needed?" className="w-full border rounded-lg px-3 py-2 text-sm" />
              </div>
            </div>
      ...
      {showApprove && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-md">
            <h2 className="text-lg font-semibold mb-4 text-gray-900">Review Overtime</h2>
            ...
            <div className="space-y-3">
              <div>
                ...
                <select value={approvalStatus} onChange={e => setApprovalStatus(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm">
              ...
              {approvalStatus === 'rejected' && (
                <div>
                  ...
                  <textarea value={rejectionReason} onChange={e => setRejectionReason(e.target.value)} rows={2} className="w-full border rounded-lg px-3 py-2 text-sm" />
                </div>
              )}
            </div>
            <div className="flex justify-end gap-2 mt-4">
              <button onClick={() => setShowApprove(null)} className="px-4 py-2 text-sm text-gray-600 hover:text-gray-800">Cancel</button>
