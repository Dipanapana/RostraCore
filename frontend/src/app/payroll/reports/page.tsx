  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
            ...
            <div className="text-2xl font-bold text-gray-900 dark:text-white">{summary.headcount}</div>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
            <div className="flex items-center gap-2 text-sm text-gray-500 mb-1"><DollarSign className="w-4 h-4" />Gross Pay</div>
            <div className="text-xl font-bold text-gray-900 dark:text-white">{fmtZAR(summary.total_gross)}</div>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
            ...
            <div className="text-xl font-bold text-gray-900 dark:text-white">{summary.total_hours}</div>
      ...
      {summary && summary.headcount > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4 max-w-xs">
      ...
      {trend?.trend?.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
          <h2 className="font-semibold text-gray-900 dark:text-white mb-4">6-Month Payroll Trend</h2>
                ...
                {trend.trend.map((t: any, i: number) => {
                  ...
                  return (
                    <tr key={t.month} className="border-t border-gray-100 dark:border-gray-700">
                      ...
                      <td className="py-2 text-right text-gray-700 dark:text-gray-300">{t.headcount}</td>
      ...
      {employees && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700">
            <h2 className="font-semibold text-gray-900 dark:text-white">Employee Breakdown — {employees.month_label}</h2>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 dark:bg-gray-700">
              <tr>
                ...
                <th className="px-4 py-3 text-right text-gray-600 dark:text-gray-300 font-medium">Hours</th>
                <th className="px-4 py-3 text-right text-gray-600 dark:text-gray-300 font-medium">OT Hrs</th>
                <th className="px-4 py-3 text-right text-gray-600 dark:text-gray-300 font-medium">Gross</th>
                <th className="px-4 py-3 text-right text-gray-600 dark:text-gray-300 font-medium">Deductions</th>
                <th className="px-4 py-3 text-right text-gray-600 dark:text-gray-300 font-medium">Net Pay</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {employees.employees.length === 0 ? (
                ...
                <tr key={e.employee_id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  <td className="px-4 py-3 font-medium text-gray-900 dark:text-white">{e.employee_name}</td>
                  <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-300">{e.total_hours}</td>
                  <td className="px-4 py-3 text-right text-orange-600">{e.overtime_hours > 0 ? e.overtime_hours : '—'}</td>
                  <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-300">{fmtZAR(e.total_gross)}</td>
