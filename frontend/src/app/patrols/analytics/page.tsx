    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Patrol Analytics</h1>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
          ...
          <div className="text-2xl font-bold text-gray-900 dark:text-white">{data.total_runs}</div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {data.by_tour.length > 0 && (
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
            <h2 className="font-semibold text-gray-900 dark:text-white mb-4">By Tour</h2>
            <div className="space-y-3">
              {data.by_tour.map((t: any) => (
                <div key={t.tour_id} className="flex items-center justify-between py-2 border-b border-gray-100 dark:border-gray-700 last:border-0">
                  <div>
                    <div className="text-sm font-medium text-gray-900 dark:text-white">{t.tour_name}</div>
        ...
        {data.by_site.length > 0 && (
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
            <h2 className="font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2"><MapPin className="w-4 h-4" />By Site</h2>
            <div className="space-y-3">
      ...
      {data.top_guards.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
          <h2 className="font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2"><Users className="w-4 h-4" />Top Patrol Guards</h2>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {data.top_guards.map((g: any, i: number) => (
                  <tr key={g.employee_id} className="border-b border-gray-100 dark:border-gray-700 last:border-0">
                    <td className="py-2">
                      <span className="w-6 h-6 inline-flex items-center justify-center rounded-full bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400 text-xs font-bold">{i + 1}</span>
                    </td>
                    <td className="py-2 font-medium text-gray-900 dark:text-white">{g.employee_name}</td>
                    <td className="py-2 text-right text-gray-700 dark:text-gray-300">{g.total_runs}</td>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {data.monthly_trend.map((t: any) => (
                  <tr key={t.month} className="border-b border-gray-100 dark:border-gray-700 last:border-0">
                    <td className="py-2 font-medium text-gray-900 dark:text-white">{t.month}</td>
                    <td className="py-2 text-right text-gray-700 dark:text-gray-300">{t.total_runs}</td>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
          ...
          <div className="space-y-2">
            {data.gap_tours.map((g: any) => (
              <div key={g.tour_id} className="flex items-center justify-between py-2 border-b border-gray-100 dark:border-gray-700 last:border-0">
                <div>
                  <div className="text-sm font-medium text-gray-900 dark:text-white">{g.tour_name}</div>
                  {g.site_name && <div className="text-xs text-gray-500">{g.site_name}</div>}
                </div>
                <span className="px-2 py-0.5 text-xs rounded-full bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400 font-medium">No runs</span>
              </div>
            ))}
          </div>
        </div>
