...
const STATUS_CONFIG: Record<string, { label: string; bg: string; text: string; border: string }> = {
  green: { label: 'Compliant', bg: 'bg-emerald-100 dark:bg-emerald-900/30', text: 'text-emerald-700 dark:text-emerald-400', border: 'border-emerald-200 dark:border-emerald-800' },
  amber: { label: 'Overtime', bg: 'bg-amber-100 dark:bg-amber-900/30', text: 'text-amber-700 dark:text-amber-400', border: 'border-amber-200 dark:border-amber-800' },
  red: { label: 'Exceeds BCEA', bg: 'bg-red-100 dark:bg-red-900/30', text: 'text-red-700 dark:text-red-400', border: 'border-red-200 dark:border-red-800' },
}
          ...
          <>
            ...
            {r.employees.length > 0 ? (
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-slate-50 dark:divide-slate-700/30">
                      {r.employees.map((emp) => {
                        ...
                        return (
                          <tr key={emp.employee_id} className="hover:bg-slate-50 dark:hover:bg-slate-700/20 transition-colors">
                            <td className="px-4 py-3 font-medium text-slate-800 dark:text-slate-200 whitespace-nowrap">
                              {emp.employee_name}
                              <span className="block text-xs text-slate-400 font-normal">#{emp.employee_number}</span>
                            </td>
                            <td className="px-4 py-3 text-slate-500 dark:text-slate-400 whitespace-nowrap capitalize">
                              {emp.role?.replace('_', ' ') ?? '—'}
                            </td>
                            <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{emp.shift_count}</td>
                            <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{emp.regular_hours}h</td>
                            <td className="px-4 py-3 whitespace-nowrap">
                              {emp.overtime_hours > 0 ? (
                                <span className="text-amber-600 dark:text-amber-400 font-medium">{emp.overtime_hours}h</span>
                              ) : (
                            ...
                            <td className="px-4 py-3 whitespace-nowrap">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="font-semibold text-slate-800 dark:text-slate-200">{emp.total_hours}h</span>
                              </div>
                            </td>
                            <td className="px-4 py-3 text-slate-400 text-xs">{emp.max_hours_week}h</td>
                            <td className="px-4 py-3 whitespace-nowrap">
              ...
              <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-8 text-center">
                ...
                <p className="text-slate-500 dark:text-slate-400">No shift assignments found for this week.</p>
              </div>
            )}
