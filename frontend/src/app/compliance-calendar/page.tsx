...
const TYPE_CONFIG: Record<string, { label: string; color: string; icon: any }> = {
  cert_expiry: { label: 'Certification', color: 'border-red-400 bg-red-50 dark:bg-red-900/20', icon: Award },
  contract_expiry: { label: 'Contract', color: 'border-orange-400 bg-orange-50 dark:bg-orange-900/20', icon: FileText },
  training_due: { label: 'Training', color: 'border-blue-400 bg-blue-50 dark:bg-blue-900/20', icon: GraduationCap },
  leave: { label: 'Leave', color: 'border-green-400 bg-green-50 dark:bg-green-900/20', icon: Calendar },
}
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4 border-l-4 border-orange-500">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <select value={daysAhead} onChange={e => setDaysAhead(Number(e.target.value))} className="border rounded-lg px-3 py-2 text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white">
                ...
                {dayEvents.map((ev: any, i: number) => {
                  ...
                  return (
                    <div key={`${dateStr}-${i}`} className={`flex items-start gap-3 border-l-4 rounded-lg p-3 ${cfg.color}`}>
                      ...
                      <span className="text-xs px-2 py-0.5 rounded-full bg-white/60 dark:bg-gray-700 text-gray-600 dark:text-gray-300 shrink-0">
                        {cfg.label}
                      </span>
                    </div>
                  )
