  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Site Profitability</h1>
      </div>
      ...
      {data && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
            ...
            <div className="text-xl font-bold text-gray-900 dark:text-white">{fmtZAR(data.total_revenue)}</div>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
            <div className="text-sm text-gray-500">Total Cost</div>
            <div className="text-xl font-bold text-gray-900 dark:text-white">{fmtZAR(data.total_cost)}</div>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
      ...
      {summary?.monthly_trend?.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
          <h2 className="font-semibold text-gray-900 dark:text-white mb-4">6-Month Trend</h2>
              ...
              <tbody>
                {summary.monthly_trend.map((t: any) => (
                  <tr key={t.month} className="border-t border-gray-100 dark:border-gray-700">
                    ...
                    <td className="py-2 text-right text-gray-700 dark:text-gray-300">{fmtZAR(t.revenue)}</td>
                    <td className="py-2 text-right text-gray-700 dark:text-gray-300">{fmtZAR(t.cost)}</td>
      ...
      {/* Filters */}
      <div className="flex items-center gap-3">
        <select value={year} onChange={e => setYear(Number(e.target.value))} className="border rounded-lg px-3 py-2 text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white">
          {[2024, 2025, 2026, 2027].map(y => <option key={y} value={y}>{y}</option>)}
        </select>
        <select value={month} onChange={e => setMonth(Number(e.target.value))} className="border rounded-lg px-3 py-2 text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white">
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 dark:bg-gray-700">
              <tr>
                ...
                <th className="px-4 py-3 text-right text-gray-600 dark:text-gray-300 font-medium">Revenue</th>
                <th className="px-4 py-3 text-right text-gray-600 dark:text-gray-300 font-medium">Labour</th>
                <th className="px-4 py-3 text-right text-gray-600 dark:text-gray-300 font-medium">OT Cost</th>
                <th className="px-4 py-3 text-right text-gray-600 dark:text-gray-300 font-medium">Total Cost</th>
                <th className="px-4 py-3 text-right text-gray-600 dark:text-gray-300 font-medium">Profit</th>
                <th className="px-4 py-3 text-right text-gray-600 dark:text-gray-300 font-medium">Margin</th>
                <th className="px-4 py-3 text-center text-gray-600 dark:text-gray-300 font-medium">Shifts</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {data.sites.length === 0 ? (
                ...
                <tr key={s.site_id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  ...
                  <td className="px-4 py-3 text-gray-600 dark:text-gray-400">{s.client_name || '—'}</td>
                  <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-300">{fmtZAR(s.monthly_revenue)}</td>
                  <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-300">{fmtZAR(s.labour_cost)}</td>
                  <td className="px-4 py-3 text-right text-orange-600">{s.overtime_cost > 0 ? fmtZAR(s.overtime_cost) : '—'}</td>
                  <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-300">{fmtZAR(s.total_cost)}</td>
