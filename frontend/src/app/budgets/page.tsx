...
const CAT_COLORS: Record<string, string> = {
  wages: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400',
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      {dashboard && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
            ...
            <div className="text-xl font-bold text-gray-900 dark:text-white">{fmtZAR(dashboard.total_allocated)}</div>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
            <div className="text-sm text-gray-500">Total Spent</div>
            <div className="text-xl font-bold text-gray-900 dark:text-white">{fmtZAR(dashboard.total_spent)}</div>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
            ...
            <div className="text-xl font-bold text-gray-900 dark:text-white">{dashboard.overall_utilisation_pct}%</div>
      ...
      {dashboard?.by_category?.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
          <h2 className="font-semibold text-gray-900 dark:text-white mb-4">Budget by Category</h2>
          <div className="space-y-3">
            {dashboard.by_category.map((c: any) => (
              <div key={c.category} className="flex items-center gap-4">
                ...
                <span className="text-sm font-medium text-gray-900 dark:text-white w-16 text-right">{c.utilisation_pct}%</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-4">
          ...
          <div className="space-y-1">
            {dashboard.over_budget_items.map((b: any) => (
              <div key={b.budget_id} className="text-sm text-red-600 dark:text-red-400">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <select value={filterYear} onChange={e => setFilterYear(Number(e.target.value))} className="border rounded-lg px-3 py-2 text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white">
          {[2024, 2025, 2026, 2027].map(y => (
            <option key={y} value={y}>{y}</option>
          ))}
        </select>
        <select value={filterCategory} onChange={e => setFilterCategory(e.target.value)} className="border rounded-lg px-3 py-2 text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white">
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 dark:bg-gray-700">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-gray-600 dark:text-gray-300 font-medium">Period</th>
              <th className="px-4 py-3 text-right text-gray-600 dark:text-gray-300 font-medium">Allocated</th>
              <th className="px-4 py-3 text-right text-gray-600 dark:text-gray-300 font-medium">Spent</th>
              <th className="px-4 py-3 text-right text-gray-600 dark:text-gray-300 font-medium">Remaining</th>
              <th className="px-4 py-3 text-right text-gray-600 dark:text-gray-300 font-medium">Use %</th>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            {items.length === 0 ? (
              ...
              <tr key={b.budget_id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                ...
                <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-300">{fmtZAR(b.allocated_amount)}</td>
                <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-300">{fmtZAR(b.spent_amount)}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md mx-4 p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white">{editing ? 'Edit Budget' : 'New Budget'}</h2>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Category</label>
                  <select value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                    {CATEGORIES.map(c => <option key={c} value={c}>{c.charAt(0).toUpperCase() + c.slice(1)}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Year</label>
                  <input type="number" value={form.period_year} onChange={e => setForm({ ...form, period_year: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Month (optional, leave blank for annual)</label>
                <select value={form.period_month} onChange={e => setForm({ ...form, period_month: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Allocated (R)</label>
                  <input type="number" step="0.01" value={form.allocated_amount} onChange={e => setForm({ ...form, allocated_amount: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" placeholder="e.g. 50000" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Spent (R)</label>
                  <input type="number" step="0.01" value={form.spent_amount} onChange={e => setForm({ ...form, spent_amount: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" placeholder="0" />
                </div>
              </div>
              <div>
