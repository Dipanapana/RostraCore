...
function reasonBadge(r: string | null) {
  const colors: Record<string, string> = {
    new_assignment: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400',
    transfer: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400',
    rotation: 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400',
    temporary: 'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300',
  }
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Guard Deployments</h1>
        </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 dark:bg-gray-700">
                <tr>
                  ...
                  <th className="px-4 py-3 text-center font-medium text-gray-600 dark:text-gray-300">Reason</th>
                  <th className="px-4 py-3 text-left font-medium text-gray-600 dark:text-gray-300">Start</th>
                  <th className="px-4 py-3 text-left font-medium text-gray-600 dark:text-gray-300">End</th>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                ...
                {records.map((r: any) => (
                  <tr key={r.record_id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <td className="px-4 py-3 text-gray-600 dark:text-gray-400">{r.start_date}</td>
                    <td className="px-4 py-3">
                      {r.end_date
                        ? <span className="text-gray-600 dark:text-gray-400">{r.end_date}</span>
                        : <span className="px-2 py-0.5 text-xs rounded-full bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400">Active</span>
                      }
                    </td>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
              ...
              <div className="text-2xl font-bold text-gray-900 dark:text-white">{dashboard.completed_deployments}</div>
            </div>
          </div>
          ...
          {dashboard.by_reason?.length > 0 && (
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
              <h3 className="font-semibold text-gray-900 dark:text-white mb-3">By Reason</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {dashboard.by_reason.map(([reason, count]: [string, number]) => (
                  <div key={reason} className="flex items-center justify-between p-2 bg-gray-50 dark:bg-gray-700 rounded-lg">
                    <span className="text-sm text-gray-700 dark:text-gray-300">{REASONS[reason] || reason}</span>
          ...
          {dashboard.by_site?.length > 0 && (
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
              <h3 className="font-semibold text-gray-900 dark:text-white mb-3">Active Deployments by Site</h3>
          ...
          {dashboard.top_employees?.length > 0 && (
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
              <h3 className="font-semibold text-gray-900 dark:text-white mb-3">Most Deployed Guards</h3>
              <div className="space-y-2">
                {dashboard.top_employees.map((e: any, i: number) => (
                  <div key={e.employee_id} className="flex items-center justify-between py-2 border-b border-gray-100 dark:border-gray-700 last:border-0">
                    <div className="flex items-center gap-2">
                      <span className="w-5 h-5 flex items-center justify-center text-xs bg-gray-200 dark:bg-gray-600 rounded-full text-gray-700 dark:text-gray-300">{i + 1}</span>
                      <span className="text-sm font-medium text-gray-900 dark:text-white">{e.employee_name}</span>
                    </div>
                    <span className="text-sm text-gray-600 dark:text-gray-400">{e.deployment_count} deployments</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <h2 className="text-lg font-bold mb-4 text-gray-900 dark:text-white">New Deployment</h2>
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="date" value={form.start_date} onChange={e => setForm({...form, start_date: e.target.value})} className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">End Date</label>
                  <input type="date" value={form.end_date} onChange={e => setForm({...form, end_date: e.target.value})} className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Role</label>
                <input type="text" value={form.role} onChange={e => setForm({...form, role: e.target.value})} placeholder="e.g. Site Supervisor, Patrol, Access Control" className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Reason</label>
                <select value={form.reason} onChange={e => setForm({...form, reason: e.target.value})} className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white">
      ...
      {showEdit && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <h2 className="text-lg font-bold mb-4 text-gray-900 dark:text-white">Edit Deployment — {showEdit.employee_name}</h2>
            <div className="space-y-3">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Site</label>
                <select value={form.site_id} onChange={e => setForm({...form, site_id: e.target.value})} className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Role</label>
                <input type="text" value={form.role} onChange={e => setForm({...form, role: e.target.value})} className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Reason</label>
                <select value={form.reason} onChange={e => setForm({...form, reason: e.target.value})} className="w-full border rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white">
