            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200 text-left">
                  <th className="px-4 py-3 font-medium text-gray-500">Visitor</th>
                  ...
                  <th className="px-4 py-3 font-medium text-gray-500">Signed In</th>
                  <th className="px-4 py-3 font-medium text-gray-500">Signed Out</th>
              ...
              <tbody>
                {visitors.length === 0 ? (
                  ...
                  visitors.map((v) => (
                    <tr key={v.visitor_id} className="border-b border-gray-100 hover:bg-gray-50">
                      <td className="px-4 py-3">
                        <div className="text-gray-900 font-medium">{v.full_name}</div>
                      ...
                      <td className="px-4 py-3 text-gray-600 max-w-[200px] truncate">{v.purpose}</td>
                      <td className="px-4 py-3">
                        <span
                      ...
                      <td className="px-4 py-3 text-gray-500 text-xs">{formatDate(v.signed_in_at)}</td>
                      <td className="px-4 py-3 text-gray-500 text-xs">{formatDate(v.signed_out_at)}</td>
                      <td className="px-4 py-3">
                        {v.status === 'signed_in' && (
                          <button
                ...
                {onSiteVisitors.length === 0 ? (
                  ...
                  onSiteVisitors.map((v) => (
                    <tr key={v.visitor_id} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-gray-600">{v.vehicle_reg || '-'}</td>
                      <td className="px-4 py-3 text-gray-500 text-xs">{formatDate(v.signed_in_at)}</td>
                      <td className="px-4 py-3">
                        <button
                          onClick={() => handleSignOut(v.visitor_id)}
      ...
      {tab === 'dashboard' && dashboard && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
              <p className="text-xs text-gray-500 mb-1">Total Visitors ({dashboard.period_days}d)</p>
              <p className="text-2xl font-bold text-gray-900">{dashboard.total_visitors}</p>
            </div>
            <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
              <p className="text-xs text-gray-500 mb-1">Currently On Site</p>
              <p className="text-2xl font-bold text-green-600">{dashboard.currently_in}</p>
            </div>
            <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
              <p className="text-xs text-gray-500 mb-1">Signed Out</p>
              <p className="text-2xl font-bold text-gray-600">{dashboard.signed_out}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
              <h3 className="font-semibold text-gray-900 mb-3">Visitors by Site</h3>
                  ...
                  {dashboard.sites.map((s) => (
                    <div key={s.site_id} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                      ...
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        {s.currently_in > 0 && (
                          <span className="px-1.5 py-0.5 rounded-full text-xs bg-emerald-50 text-emerald-700">
                            {s.currently_in} in
                          </span>
                        )}
                      </div>
            ...
            {/* Busiest Hours */}
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
              <h3 className="font-semibold text-gray-900 mb-3">Busiest Hours</h3>
                  ...
                  {dashboard.busiest_hours.map(([hour, count], idx) => (
                    <div key={idx} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-700">{formatHour(hour)}</span>
                      </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-4 border-b border-gray-200">
              <h3 className="font-semibold text-gray-900">Sign In Visitor</h3>
            ...
            <div className="p-4 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Person Being Visited</label>
                <input
                  type="text"
